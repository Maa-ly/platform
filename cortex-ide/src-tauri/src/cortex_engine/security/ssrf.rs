...
const USER_AGENT: &str = "cortex-gui/0.1";
...
mod tests {
    ...
    fn test_blocked_private_ips() {
        ...
        assert!(protection.validate_url("http://169.254.169.254").is_err());
    }
    ...
    fn test_ipv4_edge_cases() {
        ...
        assert!(protection.validate_url("http://192.0.2.1").is_err());
        assert!(protection.validate_url("http://198.51.100.1").is_err());
        assert!(protection.validate_url("http://203.0.113.1").is_err());
