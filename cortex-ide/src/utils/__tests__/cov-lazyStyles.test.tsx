...
describe("lazyStyles coverage", () => {
  it("should exercise loadStylesheet", () => {
    expect(loadStylesheet).toBeDefined();
    try {
      loadStylesheet();
    } catch (_e) {}
    try { loadStylesheet("test string", "another arg"); } catch (_e) {}
    try { loadStylesheet("hello world\nline 2\nline 3"); } catch (_e) {}
    try { loadStylesheet({}); } catch (_e) {}
    try { loadStylesheet({ key: "value", enabled: true, items: [1,2,3] }); } catch (_e) {}
  });
