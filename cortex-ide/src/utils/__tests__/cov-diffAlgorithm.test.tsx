...
import * as mod from "../diffAlgorithm";

describe("diffAlgorithm comprehensive", () => {
  const exports = Object.keys(mod).filter(k => typeof (mod as any)[k] === "function");
  
  it("should call all exported functions with various inputs", () => {
    for (const name of exports) {
      const fn = (mod as any)[name];
      try { fn(); } catch(_e) {}
      try { fn("hello", "world"); } catch(_e) {}
      try { fn("line1\nline2\nline3", "line1\nmodified\nline3"); } catch(_e) {}
      try { fn("a\nb\nc\nd", "a\nc\nd\ne"); } catch(_e) {}
      try { fn(["line1", "line2", "line3"], ["line1", "modified", "line3"]); } catch(_e) {}
      try { fn(["a", "b", "c"], ["a", "c"]); } catch(_e) {}
      try { fn(["a"], ["a", "b", "c"]); } catch(_e) {}
      try { fn([], ["a", "b"]); } catch(_e) {}
      try { fn(["a", "b"], []); } catch(_e) {}
      try { fn("", "new content"); } catch(_e) {}
      try { fn("old content", ""); } catch(_e) {}
      try { fn("same", "same"); } catch(_e) {}
      // Large input
      const big1 = Array.from({length: 50}, (_, i) => "line " + i).join("\n");
      const big2 = Array.from({length: 50}, (_, i) => i === 25 ? "CHANGED" : "line " + i).join("\n");
      try { fn(big1, big2); } catch(_e) {}
      try { fn(big1.split("\n"), big2.split("\n")); } catch(_e) {}
    }
  });
});
