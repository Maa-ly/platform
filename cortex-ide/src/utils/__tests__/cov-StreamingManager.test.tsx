...
import * as mod from "../StreamingManager";

describe("StreamingManager comprehensive", () => {
  const exports = Object.keys(mod).filter(k => typeof (mod as any)[k] === "function" || typeof (mod as any)[k] === "object");
  
  it("should exercise all exports", () => {
    for (const name of exports) {
      const exp = (mod as any)[name];
      if (typeof exp === "function") {
        // Try as constructor
        try { const inst = new exp(); } catch(_e) {}
        // Try as function
        try { exp(); } catch(_e) {}
        try { exp("test"); } catch(_e) {}
        try { exp({ onChunk: vi.fn(), onComplete: vi.fn(), onError: vi.fn() }); } catch(_e) {}
        // Try creating and using methods
        try {
          const inst = new exp({ onChunk: vi.fn(), onComplete: vi.fn(), onError: vi.fn() });
          if (inst.start) inst.start();
          if (inst.stop) inst.stop();
          if (inst.pause) inst.pause();
          if (inst.resume) inst.resume();
          if (inst.write) inst.write("chunk");
          if (inst.end) inst.end();
          if (inst.abort) inst.abort();
          if (inst.reset) inst.reset();
          if (inst.process) inst.process("data");
          if (inst.handleChunk) inst.handleChunk("chunk");
