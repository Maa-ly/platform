...
import * as mod from "../settingsMigration";

describe("settingsMigration comprehensive", () => {
  ...
  it("should call all exported functions", () => {
    for (const name of exports) {
      ...
      try { fn(); } catch(_e) {}
      try { fn({}); } catch(_e) {}
      try { fn({ "editor.fontSize": 14, "editor.tabSize": 2 }); } catch(_e) {}
      try { fn({ "editor.fontSize": 14 }, "1.0.0"); } catch(_e) {}
      try { fn({ "editor.fontSize": 14 }, "1.0.0", "2.0.0"); } catch(_e) {}
      try { fn({ "old.setting": "value" }); } catch(_e) {}
      try { fn({ "deprecated.key": true, "new.key": false }); } catch(_e) {}
      try { fn(null); } catch(_e) {}
      try { fn(undefined); } catch(_e) {}
      try { fn("string input"); } catch(_e) {}
      try { fn([]); } catch(_e) {}
    }
  });
});
