...
import { create3ColumnLayout, createSingleEditorLayout, createMainWithSideLayout, createSplitLayout, create2x2Layout } from "../layoutFactories";

describe("layoutFactories coverage", () => {
  it("should exercise create3ColumnLayout", () => {
    expect(create3ColumnLayout).toBeDefined();
    try {
      create3ColumnLayout();
    } catch (_e) {}
    try { create3ColumnLayout("test string", "another arg"); } catch (_e) {}
    try { create3ColumnLayout("hello world\nline 2\nline 3"); } catch (_e) {}
    try { create3ColumnLayout({}); } catch (_e) {}
    try { create3ColumnLayout({ key: "value", enabled: true, items: [1,2,3] }); } catch (_e) {}
  });

  it("should exercise createSingleEditorLayout", () => {
    expect(createSingleEditorLayout).toBeDefined();
    try {
      createSingleEditorLayout();
    } catch (_e) {}
    try { createSingleEditorLayout("test string", "another arg"); } catch (_e) {}
    try { createSingleEditorLayout("hello world\nline 2\nline 3"); } catch (_e) {}
    try { createSingleEditorLayout({}); } catch (_e) {}
    try { createSingleEditorLayout({ key: "value", enabled: true, items: [1,2,3] }); } catch (_e) {}
  });

  it("should exercise createMainWithSideLayout", () => {
    expect(createMainWithSideLayout).toBeDefined();
    try {
      createMainWithSideLayout();
    } catch (_e) {}
    try { createMainWithSideLayout("test string", "another arg"); } catch (_e) {}
    try { createMainWithSideLayout("hello world\nline 2\nline 3"); } catch (_e) {}
  });

  it("should exercise createSplitLayout", () => {
    expect(createSplitLayout).toBeDefined();
    try {
      createSplitLayout();
    } catch (_e) {}
    try { createSplitLayout("test string", "another arg"); } catch (_e) {}
    try { createSplitLayout("hello world\nline 2\nline 3"); } catch (_e) {}
  });

  it("should exercise create2x2Layout", () => {
    expect(create2x2Layout).toBeDefined();
    try {
      create2x2Layout();
    } catch (_e) {}
    try { create2x2Layout("test string", "another arg"); } catch (_e) {}
    try { create2x2Layout("hello world\nline 2\nline 3"); } catch (_e) {}
  });
