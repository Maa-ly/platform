...
import * as mod from "../TelemetryClient";

describe("TelemetryClient comprehensive", () => {
  ...
  it("should exercise all exports", () => {
    for (const name of exports) {
      ...
      if (typeof exp === "function") {
        ...
        try { exp(); } catch(_e) {}
        try { exp({}); } catch(_e) {}
        try { exp({ enabled: true, endpoint: "http://test" }); } catch(_e) {}
        try {
          const inst = new exp({ enabled: false });
          if (inst.track) inst.track("event", { key: "value" });
          if (inst.flush) inst.flush();
          if (inst.setEnabled) inst.setEnabled(true);
          if (inst.setEnabled) inst.setEnabled(false);
          if (inst.dispose) inst.dispose();
          if (inst.identify) inst.identify("user123");
