...
import { buildTableItems, buildConflictsMap, formatRecordedKey } from "../keybindingsHelpers";

describe("keybindingsHelpers coverage", () => {
  it("should exercise buildTableItems", () => {
    expect(buildTableItems).toBeDefined();
    try {
      buildTableItems();
    } catch (_e) {}
    try { buildTableItems("test string", "another arg"); } catch (_e) {}
    try { buildTableItems("hello world\nline 2\nline 3"); } catch (_e) {}
    try { buildTableItems(["a", "b", "c"]); } catch (_e) {}
    try { buildTableItems([{name:"test",value:"val"}]); } catch (_e) {}
  });

  it("should exercise buildConflictsMap", () => {
    expect(buildConflictsMap).toBeDefined();
    try {
      buildConflictsMap();
    } catch (_e) {}
    try { buildConflictsMap("test string", "another arg"); } catch (_e) {}
    try { buildConflictsMap("hello world\nline 2\nline 3"); } catch (_e) {}
  });

  it("should exercise formatRecordedKey", () => {
    expect(formatRecordedKey).toBeDefined();
    try {
      formatRecordedKey();
    } catch (_e) {}
    try { formatRecordedKey("test string", "another arg"); } catch (_e) {}
    try { formatRecordedKey("hello world\nline 2\nline 3"); } catch (_e) {}
    try { formatRecordedKey({}); } catch (_e) {}
    try { formatRecordedKey({ key: "value", enabled: true, items: [1,2,3] }); } catch (_e) {}
  });
