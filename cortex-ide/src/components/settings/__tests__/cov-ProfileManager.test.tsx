...
import { registerProfileCommands, ProfileManager, ProfileQuickSwitch } from "../../settings/ProfileManager";

describe("ProfileManager", () => {
  it("registerProfileCommands", () => {
    try { registerProfileCommands({} as any); } catch (_e) { /* expected */ }
    try { registerProfileCommands(); } catch (_e) { /* expected */ }
    expect(registerProfileCommands).toBeDefined();
  });
  it("ProfileManager", () => {
    try { render(() => <ProfileManager />); } catch (_e) { /* expected */ }
    expect(ProfileManager).toBeDefined();
  });
  it("ProfileQuickSwitch", () => {
    try { render(() => <ProfileQuickSwitch />); } catch (_e) { /* expected */ }
    expect(ProfileQuickSwitch).toBeDefined();
  });
});
