...
describe("SessionFilters coverage", () => {
  it("should exercise SessionFilters", () => {
    expect(SessionFilters).toBeDefined();
    try {
      const { createRoot } = require("solid-js");
      createRoot((d: any) => { try { SessionFilters({}); } catch(_e) {} d(); });
    } catch (_e) {}
    try { SessionFilters({}); } catch (_e) {}
    try { SessionFilters({ key: "value", enabled: true, items: [1,2,3] }); } catch (_e) {}
  });
