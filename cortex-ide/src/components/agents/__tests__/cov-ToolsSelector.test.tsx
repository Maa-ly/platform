...
describe("ToolsSelector coverage", () => {
  it("should exercise ToolsSelector", () => {
    expect(ToolsSelector).toBeDefined();
    try {
      const { createRoot } = require("solid-js");
      createRoot((d: any) => { try { ToolsSelector({}); } catch(_e) {} d(); });
    } catch (_e) {}
    try { ToolsSelector({}); } catch (_e) {}
    try { ToolsSelector({ key: "value", enabled: true, items: [1,2,3] }); } catch (_e) {}
  });
