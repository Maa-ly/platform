...
describe("VariablesPanel", () => {
  ...
  it("renders variables in tree structure", () => {
    mockDebug.state.isPaused = true;
    mockDebug.state.activeFrameId = 1;
    mockDebug.getScopes.mockResolvedValue([
      { name: "Local", variablesReference: 1, expensive: false },
    ]);
    mockDebug.getScopeVariables.mockResolvedValue([
      { name: "count", value: "42", type: "number", variablesReference: 0 },
      { name: "items", value: "Array(3)", type: "Array", variablesReference: 5 },
    ]);
    const { container } = render(() => <VariablesPanel />);
    expect(container.querySelector("div")).toBeTruthy();
  });
