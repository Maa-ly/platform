...
describe("MemoryInspector", () => {
  ...
  it("renders without crashing", () => {
    const { container } = render(() => <MemoryInspector />);
    expect(container.querySelector("div")).toBeTruthy();
  });

  it("shows address input", () => {
    const { container } = render(() => <MemoryInspector />);
    const input = container.querySelector('input[placeholder="Address (0x...)"]');
    expect(input).toBeTruthy();
  });

  it("shows empty state initially", () => {
    const { container } = render(() => <MemoryInspector />);
    expect(container.textContent).toContain("Enter a memory address to inspect");
  });

  it("renders hex grid header", () => {
    const { container } = render(() => <MemoryInspector />);
