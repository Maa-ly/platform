...
import { MemoryView, useMemoryViewFromVariable } from "../../debugger/MemoryView";

describe("MemoryView", () => {
  it("MemoryView", () => {
    try { render(() => <MemoryView />); } catch (_e) { /* expected */ }
    expect(MemoryView).toBeDefined();
  });
  it("useMemoryViewFromVariable", () => {
    try { createRoot((dispose) => { try { useMemoryViewFromVariable(); } catch (_e) {} dispose(); }); } catch (_e) { /* expected */ }
    expect(useMemoryViewFromVariable).toBeDefined();
  });
});
