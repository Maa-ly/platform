...
import { getOpenWithMenuItems, OpenWithMenu, useOpenWithMenu } from "../OpenWithMenu";

describe("OpenWithMenu", () => {
  it("getOpenWithMenuItems", () => {
    try { getOpenWithMenuItems("test", {} as any, {} as any); } catch (_e) { /* expected */ }
    try { getOpenWithMenuItems(); } catch (_e) { /* expected */ }
    expect(getOpenWithMenuItems).toBeDefined();
  });
  it("OpenWithMenu", () => {
    try { render(() => <OpenWithMenu />); } catch (_e) { /* expected */ }
    expect(OpenWithMenu).toBeDefined();
  });
  it("useOpenWithMenu", () => {
    try { createRoot((dispose) => { try { useOpenWithMenu(); } catch (_e) {} dispose(); }); } catch (_e) { /* expected */ }
    expect(useOpenWithMenu).toBeDefined();
  });
});
