...
import { SearchInOpenEditors, useSearchInOpenEditors } from "../../search/SearchInOpenEditors";

describe("SearchInOpenEditors", () => {
  it("SearchInOpenEditors", () => {
    try { render(() => <SearchInOpenEditors />); } catch (_e) { /* expected */ }
    expect(SearchInOpenEditors).toBeDefined();
  });
  it("useSearchInOpenEditors", () => {
    try { createRoot((dispose) => { try { useSearchInOpenEditors(); } catch (_e) {} dispose(); }); } catch (_e) { /* expected */ }
    expect(useSearchInOpenEditors).toBeDefined();
  });
});
