...
import { CollabPanel, CollabStatus } from "../../collab/CollabPanel";

describe("CollabPanel", () => {
  it("CollabPanel", () => {
    try { render(() => <CollabPanel />); } catch (_e) { /* expected */ }
    expect(CollabPanel).toBeDefined();
  });
  it("CollabStatus", () => {
    try { render(() => <CollabStatus />); } catch (_e) { /* expected */ }
    expect(CollabStatus).toBeDefined();
  });
});
