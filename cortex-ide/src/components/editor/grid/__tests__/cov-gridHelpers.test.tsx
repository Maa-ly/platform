...
import { replaceCell, getLeafCellIds, cloneCell, findCell } from "../gridHelpers";

describe("gridHelpers coverage", () => {
  it("should exercise replaceCell", () => {
    expect(replaceCell).toBeDefined();
    try {
      replaceCell();
    } catch (_e) {}
    try { replaceCell("test string", "another arg"); } catch (_e) {}
    try { replaceCell("hello world\nline 2\nline 3"); } catch (_e) {}
    try { replaceCell({}); } catch (_e) {}
    try { replaceCell({ key: "value", enabled: true, items: [1,2,3] }); } catch (_e) {}
  });

  it("should exercise getLeafCellIds", () => {
    expect(getLeafCellIds).toBeDefined();
    try {
      getLeafCellIds();
    } catch (_e) {}
    try { getLeafCellIds("test string", "another arg"); } catch (_e) {}
    try { getLeafCellIds("hello world\nline 2\nline 3"); } catch (_e) {}
    try { getLeafCellIds(["a", "b", "c"]); } catch (_e) {}
    try { getLeafCellIds([{name:"test",value:"val"}]); } catch (_e) {}
    try { getLeafCellIds({}); } catch (_e) {}
    try { getLeafCellIds({ key: "value", enabled: true, items: [1,2,3] }); } catch (_e) {}
  });

  it("should exercise cloneCell", () => {
    expect(cloneCell).toBeDefined();
    try {
      cloneCell();
    } catch (_e) {}
    try { cloneCell({}); } catch (_e) {}
    try { cloneCell({ key: "value", enabled: true, items: [1,2,3] }); } catch (_e) {}
  });

  it("should exercise findCell", () => {
    expect(findCell).toBeDefined();
    try {
      findCell();
    } catch (_e) {}
    try { findCell("test string", "another arg"); } catch (_e) {}
    try { findCell("hello world\nline 2\nline 3"); } catch (_e) {}
    try { findCell({}); } catch (_e) {}
    try { findCell({ key: "value", enabled: true, items: [1,2,3] }); } catch (_e) {}
  });
