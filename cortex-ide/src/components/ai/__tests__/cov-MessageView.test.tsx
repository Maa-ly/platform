...
import { MessageView, DateSeparator } from "../../ai/MessageView";

describe("MessageView", () => {
  it("MessageView", () => {
    try { render(() => <MessageView />); } catch (_e) { /* expected */ }
    expect(MessageView).toBeDefined();
  });
  it("DateSeparator", () => {
    try { render(() => <DateSeparator />); } catch (_e) { /* expected */ }
    expect(DateSeparator).toBeDefined();
  });
});
