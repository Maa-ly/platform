...
import { parseTokenSelector, buildTokenSelector, customizationsToMonacoRules, SemanticTokenCustomizationsProvider, useSemanticTokenCustomizations, SEMANTIC_TOKEN_TYPES, SEMANTIC_TOKEN_MODIFIERS, SEMANTIC_TOKEN_TYPE_INFO, SEMANTIC_TOKEN_MODIFIER_INFO } from "../SemanticTokenCustomizationsContext";

describe("SemanticTokenCustomizationsContext", () => {
  it("parseTokenSelector", () => {
    try { parseTokenSelector("test"); } catch (_e) { /* expected */ }
    try { parseTokenSelector(); } catch (_e) { /* expected */ }
    expect(parseTokenSelector).toBeDefined();
  });
  it("buildTokenSelector", () => {
    try { buildTokenSelector({} as any, []); } catch (_e) { /* expected */ }
    try { buildTokenSelector(); } catch (_e) { /* expected */ }
    expect(buildTokenSelector).toBeDefined();
  });
  it("customizationsToMonacoRules", () => {
    try { customizationsToMonacoRules({} as any); } catch (_e) { /* expected */ }
    try { customizationsToMonacoRules(); } catch (_e) { /* expected */ }
    expect(customizationsToMonacoRules).toBeDefined();
  });
  it("SemanticTokenCustomizationsProvider", () => {
    try { render(() => <SemanticTokenCustomizationsProvider />); } catch (_e) { /* expected */ }
    expect(SemanticTokenCustomizationsProvider).toBeDefined();
  });
  it("useSemanticTokenCustomizations", () => {
    try { createRoot((dispose) => { try { useSemanticTokenCustomizations(); } catch (_e) {} dispose(); }); } catch (_e) { /* expected */ }
    expect(useSemanticTokenCustomizations).toBeDefined();
  });
  it("SEMANTIC_TOKEN_TYPES", () => {
    expect(SEMANTIC_TOKEN_TYPES).toBeDefined();
  });
  it("SEMANTIC_TOKEN_MODIFIERS", () => {
    expect(SEMANTIC_TOKEN_MODIFIERS).toBeDefined();
  });
  it("SEMANTIC_TOKEN_TYPE_INFO", () => {
    expect(SEMANTIC_TOKEN_TYPE_INFO).toBeDefined();
  });
  it("SEMANTIC_TOKEN_MODIFIER_INFO", () => {
    expect(SEMANTIC_TOKEN_MODIFIER_INFO).toBeDefined();
  });
});
